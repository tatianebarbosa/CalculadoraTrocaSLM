(function () {
  ...
  const formEls = {
    ...
    jurosValor: document.getElementById("jurosValor"),
  ...
  function selectedState() {
    return {
      ...
      jurosValor: clampCurrency(formEls.jurosValor.value),
  ...
  function calculateState() {
    ...
    const jurosCredito = state.jurosValor;
    const creditoTotal = principal.paidMaterials + jurosCredito;
    const diferenca = Math.max(nova.paidMaterials - creditoTotal, 0);
    const saldo = Math.max(creditoTotal - nova.paidMaterials, 0);
    const podeTrocar = saldo <= 0;
  ...
  function principalRows(calc) {
    return [
      ...
      ["Crédito de juros na loja", calc.jurosCredito],
  ...
  function financialRows(calc) {
    return [
      ...
      ["Valor dos juros que ficará disponível na loja", calc.jurosCredito],
  ...
  function buildJurosSentence(calc) {
    return calc.jurosCredito > 0
      ? ` Além disso, há ${formatMoney(calc.jurosCredito)} de juros disponíveis como crédito financeiro.`
      : "";
  }
  ...
  function renderTableRows(target, rows) {
    target.innerHTML = rows
      .map(([label, value]) => `<tr><td>${label}</td><td>${formatMoney(value)}</td></tr>`)
      .join("");
  }
  ...
  function buildSimpleSummary(calc) {
    if (calc.podeTrocar) {
      ...
      return `Pode seguir com a troca. Pedido principal: ${calc.state.principalTurma}. Nova compra: ${calc.state.novaTurma}. ${diffText} Após a confirmação da troca, aguardar 24 horas e depois ajustar a matrícula no LEX para liberar o valor na loja.`;
    }

    return `A troca não pode seguir. Pedido principal: ${calc.state.principalTurma}. Nova compra: ${calc.state.novaTurma}. Como sobraria ${formatMoney(calc.saldo)} na loja, será necessário cancelar o pedido principal e refazer a matrícula correta no LEX após 24 horas.`;
  }
  ...
  function buildSchoolMessage(calc) {
    ...
    if (calc.podeTrocar) {
      ...
      return `Vamos seguir com a troca neste caso. O pedido principal foi realizado para a turma ${calc.state.principalTurma}, no valor de ${principalAmount}, e a nova compra será para a turma ${calc.state.novaTurma}, no valor de ${formatMoney(calc.nova.paidMaterials)}.${jurosSentence} Após a análise da composição do pedido principal, da nova compra e do valor efetivamente pago, identificamos que é possível seguir com a troca neste cenário. ${paymentSentence} Após a confirmação da troca, será necessário aguardar 24 horas e, depois desse prazo, a escola deverá realizar o ajuste da matrícula do(a) aluno(a) na turma correta dentro do LEX. Somente após esse ajuste o valor ficará disponível na loja para realização da nova compra. Antes desse prazo, não deve ser feita nenhuma alteração no LEX. No momento da compra, orientem o responsável a verificar se o valor foi abatido corretamente antes de finalizar o pedido.`;
    }

    return `Não será possível seguir com a troca neste caso. O pedido principal foi realizado para a turma ${calc.state.principalTurma}, no valor de ${principalAmount}, e a nova compra seria para a turma ${calc.state.novaTurma}, no valor de ${formatMoney(calc.nova.paidMaterials)}.${jurosSentence} Após a análise da composição do pedido principal, da nova compra e do valor efetivamente pago, identificamos que a nova operação geraria saldo remanescente na loja. Como sobraria o valor de ${formatMoney(calc.saldo)} na loja, esse saldo não pode ficar disponível na loja. Por esse motivo, será necessário seguir com o cancelamento do pedido principal. Após esse cancelamento, será necessário aguardar 24 horas. Depois desse prazo, a escola deverá realizar a matrícula do(a) aluno(a) na turma correta dentro do LEX e, somente após esse ajuste, o material correto ficará disponível na loja para uma nova compra. Neste caso, o responsável deverá realizar a compra integral do novo material, conforme o valor do novo pedido.`;
  }
  ...
  function buildGuardianMessage(calc) {
    ...
    if (calc.podeTrocar) {
      ...
      return `Vamos seguir com a troca do material neste caso. O pedido principal foi realizado para a turma ${calc.state.principalTurma}, no valor de ${principalAmount}, e a nova compra será para a turma ${calc.state.novaTurma}, no valor de ${formatMoney(calc.nova.paidMaterials)}.${jurosSentence} Após a análise da composição do pedido principal, da nova compra e do valor efetivamente pago, identificamos que é possível seguir com a troca neste cenário. ${paymentSentence} Após a confirmação da troca, será necessário aguardar 24 horas e, depois desse prazo, avisar a escola para que ela realize o ajuste da matrícula na turma correta dentro do LEX. Somente após esse ajuste o valor ficará disponível na loja para realização da nova compra. Antes de finalizar o pedido, orientamos que seja verificado se o valor foi abatido corretamente.`;
    }

    return `Não será possível seguir com a troca do material neste caso. O pedido principal foi realizado para a turma ${calc.state.principalTurma}, no valor de ${principalAmount}, e a nova compra seria para a turma ${calc.state.novaTurma}, no valor de ${formatMoney(calc.nova.paidMaterials)}.${jurosSentence} Após a análise da composição do pedido principal, da nova compra e do valor efetivamente pago, identificamos que a nova operação geraria saldo remanescente na loja. Como sobraria o valor de ${formatMoney(calc.saldo)} na loja, esse saldo não pode ficar disponível na loja. Por esse motivo, será necessário seguir com o cancelamento do pedido principal. Após esse cancelamento, será necessário aguardar 24 horas. Depois desse prazo, pedimos que entre em contato com a escola para que ela realize a matrícula na turma correta dentro do LEX e, somente após esse ajuste, o material correto ficará disponível na loja para uma nova compra. Neste caso, será necessário realizar a compra integral do novo material, conforme o valor do novo pedido.`;
  }
