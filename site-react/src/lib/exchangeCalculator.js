...
function buildQuickOutcome(calc) {
  ...
  if (calc.requiresCancellationForJuros) {
    return "A troca não pode seguir neste momento, pois há juros no pedido principal. Nesse cenário, será necessário cancelar o pedido principal e aguardar o respectivo reembolso antes de seguir com a troca.";
  }
...
function buildRuleUsed(calc) {
  ...
  if (calc.jurosCredit > 0) {
    ruleParts.push(
      "Aviso: neste momento, quando houver juros no pedido principal, a troca não pode seguir. Nesse cenário, será necessário cancelar o pedido principal e aguardar o respectivo reembolso antes de seguir com a nova compra."
    );
  }
...
function buildReason(calc) {
  ...
  if (calc.requiresCancellationForJuros) {
    return "A troca não pode seguir neste momento, pois há juros no pedido principal. Nesse cenário, o pedido principal precisa ser cancelado e seguir para reembolso antes da nova compra.";
  }
...
function buildSimpleSummary(calc) {
  ...
  if (calc.requiresCancellationForJuros) {
    return `A troca não pode seguir neste momento. Pedido principal: ${calc.form.principalTurma}. Nova compra: ${calc.form.novaTurma}. Como há juros no pedido principal, será necessário cancelar o pedido principal e aguardar o respectivo reembolso. Depois disso, aguardar 24 horas, ajustar a matrícula no LEX e só então seguir com a nova compra.`;
  }
